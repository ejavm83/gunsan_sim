...
`python main.py` 실행 시 콘솔에는 먼저 SimPy 엔진 단계(설정 확인 → 환경 초기화 →
입고·선별·압착·반사로·출하 프로세스 등록 → `env.run`으로 가상 시간 전진 →
이벤트 건수 등 요약)가 순서대로 안내된 뒤, KPI 표와(기본값이면) CP-SAT 비교·
차트 저장 메시지가 이어진다. 웹 대시보드(`streamlit run webapp.py`)에서는
시뮬레이션 실행 버튼을 누르면 접이식 상태창에 같은 단계가 표시된다.
...
| 파일                       | 내용                                            |
| -------------------------- | ----------------------------------------------- |
| `out/buffer_levels.png`    | 파레트 / 큐프레이크 / SCR 코일 버퍼 점유 시계열 |
...
생성하므로, 운영자가 코드를 들여다보지 않고도 시나리오 비교를 할 수 있다.
...
- 사이드바 파라미터 패널: 시뮬레이션 일수, 트럭/일, 압착기 대수, 반사로
  대수, 큐프레이크 비율 등 주요 파라미터를 슬라이더로 조정
...
- 공장 평면도(상단 큰 영역) : 입고 → 계근 → 하역 → 선별/압착 → 파레트
  버퍼 → 엘리베이터 → 반사로 1·2 → 주조 라인(큐프레이크/SCR) → 완제품
  ...
  - 반사로 박스 색상은 상태별로 회색(idle) / 노랑(charging) / 빨강(melting,
    ...
    한눈에 들어온다.
- 하단 시계열 미니 차트 3개 : 파레트 버퍼 / 큐프레이크 야적 / SCR 야적의
